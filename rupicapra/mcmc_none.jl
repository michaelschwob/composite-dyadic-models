...
function mcmc_none(y, X, H, K, phi_list, R, nMCMC, dt, ds)
    ...
    for k in ProgressBar(2:nMCMC)
        ...
        if(k % 100 == 0) # every 100 iterations, save the information
            @save "none/MCMCoutput.jld2" k betaSave thetaSave etaSave s2_ySave s2_etaSave s2_thetaSave gammaSave phiSave phiIdxSave
        end
