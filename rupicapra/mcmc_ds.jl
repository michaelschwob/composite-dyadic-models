...
#### Using Restricted Spatial Regression (RSR)
#### With Mevin's Proposal of H
####
############################################################

function mcmc_ds(y, X, H, K, phi_list, R, nMCMC, dt, ds)
    ...
    M = 5 # seems like an appropriate global maximal parameter value for ls
    ...
    for k in ProgressBar(2:nMCMC)
        ...
        gammaStar = zeros(2)
        for indgamma in 2:2 # only care for gamma_2
        ...
        ## M-H ratio
        mh1 = logpdf(MvNormal(vec(X*beta + K*eta + H*theta), Hermitian(s2_y*Sig_y_star)), y) + logpdf(Gamma(alpha_gamma, beta_gamma), gammaStar[2])
        mh2 = logpdf(MvNormal(vec(X*beta + K*eta + H*theta), Hermitian(s2_y*Sig_y)), y) + logpdf(Gamma(alpha_gamma, beta_gamma), gamma[2])
        mh = exp(mh1 - mh2)
        ...
        if(k % 100 == 0) # every 100 iterations, save the information
            @save "ds/MCMCoutput.jld2" k betaSave thetaSave etaSave s2_ySave s2_etaSave s2_thetaSave gammaSave phiSave phiIdxSave
        end
