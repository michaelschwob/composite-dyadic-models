...
#### (Temporal Lags in Composite Weights)
####
############################################################

function mcmc_dt(y, X, H, K, phi_list, R, nMCMC, dt, ds)
    ...
    for k in ProgressBar(2:nMCMC)
        ...
        gammaStar = zeros(2)
        for indgamma in 1:1 # only care for gamma_1
        ...
        ## M-H ratio
        mh1 = logpdf(MvNormal(vec(X*beta + K*eta + H*theta), Hermitian(s2_y*Sig_y_star)), y) + logpdf(Gamma(alpha_gamma, beta_gamma), gammaStar[1])
        mh2 = logpdf(MvNormal(vec(X*beta + K*eta + H*theta), Hermitian(s2_y*Sig_y)), y) + logpdf(Gamma(alpha_gamma, beta_gamma), gamma[1])
        mh = exp(mh1 - mh2)
        ...
        if(k % 100 == 0) # every 100 iterations, save the information
            @save "dt/MCMCoutput.jld2" k betaSave thetaSave etaSave s2_ySave s2_etaSave s2_thetaSave gammaSave phiSave phiIdxSave
        end
